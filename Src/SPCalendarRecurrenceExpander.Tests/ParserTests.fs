...
[<Fact>]
let ``parse Outlook created appointment with distinct apostrophe character``() =
    a.["RecurrenceData"] <- """<recurrence><rule><firstDayOfWeek>mo</firstDayOfWeek><repeat><daily dayFrequency='1' /></repeat><windowEnd>2014-08-30T00:00:00Z</windowEnd></rule></recurrence>"""
    let p = sut.Parse(a)
    test <@ p.Recurrence = Daily (EveryNthDay 1, ExplicitEnd(DateTime(2014, 8, 30, 0, 0, 0))) @>
